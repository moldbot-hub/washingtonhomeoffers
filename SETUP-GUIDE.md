# Setup Guide — Washington Home Offers Websites
...
3. Upload the site folder (e.g., `washingtonhomeoffers/`)
4. Set production branch / upload
5. Go to Custom domains → Add domain
6. Add `washingtonhomeoffers.com` and `www.washingtonhomeoffers.com`
7. Cloudflare will auto-provision SSL certificate
8. Verify site loads at https://washingtonhomeoffers.com

### DNS (at Namecheap or wherever domains are registered):
- Point nameservers to Cloudflare (if not already)
- Or add CNAME record: `www` → `washingtonhomeoffers.pages.dev`
...
3. Create an API key, restrict to:
   - HTTP referrers: `washingtonhomeoffers.com/*`, `washingtonhomes4cash.com/*`, `washingtonhomeoffers.com/*`
...
### Domain 1: washingtonhomeoffers.com

1. Go to https://business.google.com → "Add your business"
2. **Business name:** Washington Home Offers
...
10. **Description:**
```
Washington Home Offers is a local real estate investment company serving homeowners across Washington State. We buy houses in any condition — no repairs needed, no agent commissions, no hidden fees.
...
Get your instant cash offer at washingtonhomeoffers.com — see your comps and your offer in under 60 seconds.
```
...
- **Description:** Same but replace "Washington Home Offers" with "Washington Homes 4 Cash" and domain with washingtonhomes4cash.com
...
1. Copy the `washingtonhomeoffers/` folder
2. Find-and-replace in all HTML files:
   - "Washington Home Offers" → new brand name
   - "washingtonhomeoffers.com" → new domain
   - "offers@washingtonhomeoffers.com" → new email
